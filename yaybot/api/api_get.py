...
def get_hima_users(self, amount=None):
    ...
    self.logger.info('Getting hima users...')
...
def get_letters(self, user_id, amount=None):
    ...
    with tqdm(total=reviews_count, desc=f'Getting letters of {user_id}') as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_joined_groups(self, user_id, amount=100):
    self.logger.info(f"Getting groups '{user_id}' joined...")
    resp = self._get(
...
def get_user_followers(self, user_id, amount=None):
    ...
    with tqdm(total=followers_count, desc=f'Getting followers of {user_id}') as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 50)
...
def get_user_followings(self, user_id, amount=None):
    ...
    with tqdm(total=followings_count, desc=f'Getting followings of {user_id}') as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 50)
...
def get_follow_requests(self, amount):
    ...
    self.logger.info('Getting users requested to follow you...')
    resp = self._get(
...
def get_blocked_users(self, amount):
    self.logger.info('Getting users you blocked...')
    resp = self._get(
...
def get_blocked_by(self, amount):
    self.logger.info('Getting user IDs blocked you...')
    resp = self._get(
...
def get_timeline(self, amount=100):
    ...
    with tqdm(total=posts_count, desc='Getting posts of public timeline') as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_user_timeline(self, user_id, amount=None):
    ...
    with tqdm(total=posts_count, desc=f'Getting posts of {user_id}') as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_timeline_by_keyword(self, keyword, amount=100):
    ...
    with tqdm(total=posts_count, desc=f"Getting timeline by keyword of '{keyword}'") as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_timeline_by_hashtag(self, hashtag, amount=100):
    ...
    with tqdm(total=posts_count, desc=f"Getting timeline by hashtag of '{hashtag}'") as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_following_timeline(self, amount=100):
    ...
    with tqdm(total=posts_count, desc=f'Getting posts of private timeline') as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_conversation(self, conversation_id=None, post_id=None, amount=100):
    ...
    self.logger.info(f"Getting conversation of {conversation_id}...")
    resp = self._get(
...
def get_reposts(self, post_id, amount=None):
    ...
    with tqdm(total=posts_count, desc=f"Getting reposts of '{post_id}'") as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_post_likers(self, post_id, amount=None):
    ...
    with tqdm(total=likes_count, desc=f"Getting likers of '{post_id}'") as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 50)
...
def get_group_timeline(self, group_id, amount=100):
    ...
    with tqdm(total=posts_count, desc=f"Getting group posts of '{group_id}'") as pbar:
        pbar.update(number)
        while next_id and amount > 0:
            number = min(amount, 100)
...
def get_group_call(self, group_id):
    self.logger.info(f"Getting active calls in group of {group_id}...")
    resp = self._get(
...
def get_group_members(self, group_id, amount=100):
    self.logger.info(f"Getting group members of {group_id}...")
    resp = self._get(
...
def get_pending_users_in_group(self, group_id, amount=100):
    self.logger.info(f"Getting pending users of {group_id}...")
    resp = self._get(
...
def get_banned_user_from_group(self, group_id, amount=100):
    ...
    self.logger.info(f"Getting banned users of {group_id}...")
...
def get_chat_messages(self, chatroom_id=None, user_id=None, amount=None):
    if chatroom_id:
        self.logger.info(
            f"Getting messages by chatroom ID of {chatroom_id}...")
        resp = self._get(
            f'{ep.CHATROOM_v2}/{chatroom_id}/messages?number={amount}')
    if user_id:
        self.logger.info(f"Getting messages from {user_id} ...")
...
def get_chat_rooms(self, amount=None):
    self.logger.info('Getting chatrooms...')
    resp = self._get(
...
def get_chat_requests(self, amount=None):
    self.logger.info('Getting chat requests...')
    resp = self._get(
...
def get_notification(self, important=True, amount=100):
    if important:
        self.logger.info('Getting important notifications...')
        resp = self._get(
            f'{ep.CAS_URL}/api/user_activities?important=true&number={amount}')
    else:
        self.logger.info('Getting unimportant notifications...')
        resp = self._get(
