...
def _get(self, endpoint: str, data: dict = None, headers: dict = None):
    ...
    resp = httpx.get(endpoint, params=data, headers=self.headers,
...
def _post(self, endpoint: str, data: dict = None, headers: dict = None):
    if headers is None:
        headers = self.headers
    resp = httpx.post(endpoint, params=data, headers=self.headers,
                      proxies=self.proxies, timeout=self.timeout)
    _handle_response(resp)
    return resp.json()
...
def _put(self, endpoint: str, data: dict = None, headers: dict = None):
    if headers is None:
        headers = self.headers
    resp = httpx.put(endpoint, params=data, headers=self.headers,
                     proxies=self.proxies, timeout=self.timeout)
    _handle_response(resp)
    return resp.json()
...
def _delete(self, endpoint: str, data: dict = None, headers: dict = None):
    if headers is None:
        headers = self.headers
    resp = httpx.delete(endpoint, params=data, headers=self.headers,
                        proxies=self.proxies, timeout=self.timeout)
    _handle_response(resp)
    return resp.json()
